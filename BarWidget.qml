...
BarWidget {
  ...
  readonly property string tooltip: {
    ...
    if (service.isDefaultAgent)
      bits.push("default agent")
    if (service.connectedPlatforms && service.connectedPlatforms.length > 0)
      bits.push(service.connectedPlatforms.join(", "))
    if (service.working)
      bits.push("working")
