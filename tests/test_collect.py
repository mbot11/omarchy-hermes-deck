...
class InventoryTests(unittest.TestCase):
    ...
    def test_inventory_reports_cron_jobs_with_paused_state(self) -> None:
        ...
        jobs = snap["cron"]
        self.assertEqual(len(jobs), 6, f"expected 6 jobs, got {jobs!r}")
    ...
    def test_banner_text_is_not_reported_as_a_job(self) -> None:
        ...
        self.assertEqual(names[:5], ["nightly-backup", "weekly-digest",
                                 "external-tool-job", "hand-edited-record", "[urgent]"],
                         f"banner text or a label leaked into the names: {names!r}")
        # the 6th is the unnamed job, whose display name IS its prompt
        self.assertTrue(len(names) == 6 and names[5].startswith("ok"),
                        f"the unnamed job is missing or wrong: {names!r}")
    ...
    def test_fast_path_carries_cached_inventory_without_a_subprocess(self) -> None:
        ...
        self.assertEqual(len(slow["cron"]), 6)
    ...
    def test_cron_parser_reads_the_real_output_format(self) -> None:
        ...
        self.assertEqual([j["name"] for j in jobs][:5],
                         ["nightly-backup", "weekly-digest", "external-tool-job",
                          "hand-edited-record", "[urgent]"],
                         f"wrong names parsed: {jobs!r}")
        # A 6th job: unnamed, so the CLI prints its multi-line prompt as the name.
        self.assertEqual(len(jobs), 6, f"expected 6 jobs, got {jobs!r}")
    ...
    def test_round_trip_against_the_generated_fixture(self) -> None:
        ...
        self.assertEqual(jobs, [
            ...
            # An unnamed job: the CLI prints its multi-line PROMPT as the Name
...
class TestCronParserAgainstRealCliOutput(unittest.TestCase):
    """The parser must survive what the CLI actually prints.

    The fixture is generated by calling the CLI's own `cron_list` (see
    tests/fixtures/generate-cron-fixture.py), so these assertions are about the
    real format, not a hand-written idea of it. Every previous version of this
    parser was validated against something typed by hand and was wrong for
    anyone who had jobs.
    """
    ...
    def _module(self):
        import importlib.machinery
        import importlib.util
        loader = importlib.machinery.SourceFileLoader("deck_collect_real", str(COLLECT))
        spec = importlib.util.spec_from_loader("deck_collect_real", loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
        return module

    def test_fixture_yields_every_real_job(self):
        jobs = self._module().parse_cron_list(self.CRON_FIXTURE.read_text(encoding="utf-8"))
        # Six jobs. The last one's name is the multi-line PROMPT, because that is
        # genuinely what the CLI prints for an unnamed job (cron/jobs.py:497-507
        # derives the name from the prompt). Assert the structure, not a prettified
        # name: the point is that no EXTRA job appears and none is lost.
        self.assertEqual(len(jobs), 6, "job count does not match the CLI's output")
        self.assertEqual([j["name"] for j in jobs][:5],
                         ["nightly-backup", "weekly-digest", "external-tool-job",
                          "hand-edited-record", "[urgent]"])
        self.assertTrue(jobs[5]["name"].startswith("ok"),
                        "the unnamed job's prompt should be its display name")

    def test_a_multiline_prompt_name_cannot_fabricate_a_job(self):
        """An unnamed job's prompt is printed as the Name value, newlines and all.

        A prompt containing an indented `[badge]` line renders what looks exactly
        like a job header, and the block after it carries a Name AND a Schedule —
        so a flush guard requiring both does not help. The old parser reported a
        FABRICATED job ("evil", paused) and dropped the real one.
        """
        jobs = self._module().parse_cron_list(self.CRON_FIXTURE.read_text(encoding="utf-8"))
        names = [j["name"] for j in jobs]
        self.assertNotIn("evil", names, "a prompt line fabricated a job name")
        ok = [j for j in jobs if j["name"].startswith("ok")]
        self.assertEqual(len(ok), 1, "the real job was dropped")
        self.assertEqual(ok[0]["schedule"], "0 3 * * *",
                         "the real job lost its schedule to the injected block")
        self.assertFalse(ok[0]["paused"],
                         "an injected [paused] line paused a job that is active")
